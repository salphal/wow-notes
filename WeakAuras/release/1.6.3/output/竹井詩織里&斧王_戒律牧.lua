--[[
aura id: 竹井詩織里&斧王_戒律牧
aura uid: cTsdu3zUp5k
regionType: empty
...
function (event, ...)
    ...
    local t = _G.ShioriPriestLastCastUnits
    if event == "UNIT_SPELLCAST_SENT" then
        -- SENT 事件有 4 个参数
        local unit, target, castGUID, spellID = ...
        if unit == "player" then
            if spellID == 53007 then
                t.LastCastPenanceUnitName = target
                t.PenanceGUID = castGUID -- 记录这次施法的唯一 ID
                t.PenanceStartTime = GetTime() -- 记录施法开始时间
            elseif spellID == 48071 then
                t.LastCastFlashUnitName = target
                t.FlashGUID = castGUID
            end
        end
    elseif event == "UNIT_SPELLCAST_SUCCEEDED" then
        -- 【核心修复】SUCCEEDED 只能解开快速治疗，不能解开苦修！
        -- 因为苦修在这里刚刚开始引导。
        local unit, castGUID, spellID = ...
        if unit == "player" then
            if t.FlashGUID and castGUID == t.FlashGUID then
                t.LastCastFlashUnitName = ""
                t.FlashGUID = ""
            end
        end
    elseif event == "UNIT_SPELLCAST_FAILED" or event == "UNIT_SPELLCAST_INTERRUPTED" then
        ...
        local unit, castGUID, spellID = ...
        if unit == "player" then
            if t.PenanceGUID and castGUID == t.PenanceGUID then
                t.LastCastPenanceUnitName = ""
                t.PenanceGUID = ""
                t.PenanceStartTime = 0
            elseif t.FlashGUID and castGUID == t.FlashGUID then
                t.LastCastFlashUnitName = ""
                t.FlashGUID = ""
            end
        end
    elseif event == "UNIT_SPELLCAST_CHANNEL_STOP" then
        -- 【核心修复】苦修引导结束（或被打断退出引导）时，彻底解开苦修锁
        local unit, castGUID, spellID = ...
        if unit == "player" then
            -- 双保险匹配：如果是当前追踪的 GUID，或者是苦修这个法术停了，就解锁
            if (t.PenanceGUID and castGUID == t.PenanceGUID) or spellID == 53007 then
                t.LastCastPenanceUnitName = ""
                t.PenanceGUID = ""
                t.PenanceStartTime = 0
            end
